import { HangarUploadReport as UploadReport, HangarUploadRequest as UploadRequest } from "@/action";
...
import { HangarApiClient } from "./hangar-api-client";

/**
 * Configuration options for the uploader, tailored for use with Modrinth.
 */
export type HangarUploaderOptions = GenericPlatformUploaderOptions;

/**
 * Defines the structure for an upload request, adapted for use with Hangar.
 */
export type HangarUploadRequest = UploadRequest;

/**
 * Specifies the structure of the report generated after a successful upload to Hangar.
 */
export type HangarUploadReport = UploadReport;

/**
 * Implements the uploader for Modrinth.
 */
export class HangarUploader extends GenericPlatformUploader<HangarUploaderOptions, HangarUploadRequest, HangarUploadReport> {
    /**
     * Constructs a new {@link HangarUploader} instance.
     *
     * @param options - The options to use for the uploader.
     */
    constructor(options?: HangarUploaderOptions) {
        super(options);
    }
    ...
    get platform(): PlatformType {
        return PlatformType.HANGAR;
    }

    /**
     * @inheritdoc
     */
    protected async uploadCore(request: HangarUploadRequest): Promise<HangarUploadReport> {
        ArgumentError.throwIfNullOrEmpty(request.namespace, "request.namespace");

        const api = new HangarApiClient({ apiKey: request.apiKey.unwrap() });
